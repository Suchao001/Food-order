...
export default defineEventHandler(async (event) => {
    ...
    try {
        // 1. Get the menu item to find the image_url
        const checkResult = await query('SELECT image_url FROM menus WHERE id = $1', [id]);
        ...
        if (imageUrl) {
            ...
            if (publicId) {
                await deleteFromCloudinary(publicId).catch(err => {
                    ...
                    // Proceed with DB deletion even if image deletion fails
                });
            }
        }
